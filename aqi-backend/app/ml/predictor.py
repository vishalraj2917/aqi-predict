...
def _load(horizon: int):
    ...
    meta_path = ML_DIR / f"model_meta_{suffix}.pkl"
    is_xgb = joblib.load(meta_path)["is_xgb"] if meta_path.exists() else False

    if is_xgb:
        model = xgb.XGBRegressor()
        model.load_model(ML_DIR / f"aqi_forecast_{suffix}.json")
    else:
