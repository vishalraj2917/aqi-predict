...
trained XGBoost models from Phase 6 (`app/ml/aqi_forecast_h1.json` and
`aqi_forecast_h24.json`) — this is a real, working prediction, not mocked.

**Note on model file format:** the forecast models are saved as XGBoost's
native `.json` format, not pickled with joblib. Pickling an XGBoost booster
is only reliably portable on the exact machine/OS that created it — it
broke when I moved this project from Linux (where I built it) to Windows,
even with identical XGBoost version numbers on both sides. `save_model()`/
`load_model()` is the officially supported cross-platform path, so that's
what `app/ml/predictor.py` uses now.
